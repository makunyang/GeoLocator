...
class EnhancedFuzzyGeoLocator:
    ...
    def process_all_points(self, data=None):
        ...
        for i, point_data in enumerate(data['points']):
            print(f"\n {i + 1}/{len(data['points'])}: {point_data['id']}")
...
def main():
    ...
    if results:
        print("\n" + "=" * 60)
        print("Fuzzy Geolocation System - Gaussian Attenuation Version")
        print("=" * 60)
        ...
        if areas:
            print(f"Average confidence region area: {np.mean(areas):.1f} sq.m")
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print(f"  Final coordinates: ({results[0]['final_coordinate'][0]:.6f}, "
              f"{results[0]['final_coordinate'][1]:.6f})")
        print(f"  confidence coefficient: {results[0].get('confidence', 0):.3f}")
        print(f"  Number of reference objects: {results[0]['distribution_count']}")
